...
COMPANY_DOMAIN = 'company-name-here.com'
EXCEPTION_PATTERNS = {
    ...
    "document title p*": {
        ...
        "allowed_until": datetime.datetime(year=2025, month=6, day=2),
    },
}
...
def rule(event):
    ...
    if (application_name == "drive" and events and
        ...
        return any(
            _check_acl_change_event(event.udm('actor_email'), acl_change_event)
...
def title(event):
    events = event.get("events", [])
    actor_email = event.udm('actor_email')
    matching_events = [
